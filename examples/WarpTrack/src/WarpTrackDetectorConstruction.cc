...
// The WarpTrack example geometry header describes the nominal outside
// dimensions of each wrapped scintillator. The sensitive plastic is inset from every nominal
...
G4VPhysicalVolume* DetectorConstruction::BuildGeometry() {
  ...
  // device from the WarpTrack example geometry header. The outer logical volume is the
  ...
  if (serverModelEnabled) {
    ...
    for (const auto &server : servers) {
      ...
      const std::string interiorName =
          "G4Cosmic_ServerInterior_" + std::string(type.name);
